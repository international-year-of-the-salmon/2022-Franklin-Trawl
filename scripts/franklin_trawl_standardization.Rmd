...
Franklin_event <- Franklin_event %>%
  mutate(TRIP_ID = "IYS2022",
         Vessel_Name_Abbr = "Franklin",
         Station_Event_ID = paste(TRIP_ID, Vessel_Name_Abbr, Zone, Event_Type, Station, sep = "-"))
...
occurrence_allCatch$scientificname <- gsub("Hormiphora cucumisÂ ", "Hormiphora cucumis", occurrence_allCatch$scientificname)                        
...
# TODO: Change the Station_Event_IDs and Catch_IDs accordingly to ensure proper nesting under the Event Core.
...
# TODO: Change EventIDs and OccurrenceIDs to facilitate integration and nesting with Event Core. 
...
# TODO: Look at resourceRelationship again. 
...
Franklin2022_ind_eMOF <- Franklin_occurrence_specimen %>%
  select(eventID = Station_Event_ID,
         occurrenceID = Specimen_ID, 
         Ageclass, Specimen_Length, Specimen_Weight, Sex) %>%
  mutate_all(as.character) %>%
  ...
  mutate(measurementID = paste(occurrenceID, measurementType, sep = "-"),
         ...
         measurementUnit = case_when(
           measurementType == "Specimen_Length" ~ "millimeters",
